  ...
  <div class="anexo-wrap">
    <div class="anexo-centered">
      <header class="anexo-header">
        <h2>Registrar Anexo <AyudaDialog v-bind="ayudas.registrarAnexoDesdeListado" /></h2>
      </header>
...
async function indexar() {
  ...
  const tamanioArchivo = file?.size ?? null
  ...
  try {
   ...
   // const headers = {}
    ...
    const response = await api.post('/api/registerAnexo', fd, { headers })
    ...
    try {
      await guardarRendimiento(
        {
          accion: 'REGISTRAR_ANEXO_DESDE_LISTADO_DE_ACTAS',
          ...
          ui_principal: ui.value || null,
          tamanio_archivo_bytes: tamanioArchivo
        },
        headers
      )
    } catch (logError) {
      console.error('No se pudo guardar rendimiento del registro de anexo post indexación:', logError)
    }
    busy.value = false
  }
}
